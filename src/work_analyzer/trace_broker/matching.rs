...
impl ExtractedElements {
    ...
    pub async fn run_matching(self) -> Result<MatchedElements, PpdcError> {
        ...
        let mut matching_placeholders = self
            .elements
            .iter()
            .map(|element| {
                MatchedElement {
                    ...
                    landmark_suggestions: element.landmark_suggestions.iter().map(|suggestion| suggestion.into()).collect::<Vec<LandmarkMatching>>(),
                }
            })
            .collect::<Vec<MatchedElement>>();

        for landmark_type in landmark_types {
            let landmarks = self.context.landmarks.iter().filter(|landmark| landmark.landmark_type == landmark_type).cloned().collect::<Vec<Landmark>>();
            let landmark_match_placeholders = matching_placeholders
                .iter()
                .cloned()
                .filter_map(|element| {
                    element.landmark_suggestions
                        .iter()
            ...
            matching_placeholders = matching_placeholders
                ...
                .map(|element| {
                    MatchedElement {
                        ...
                        evidence: element.evidence.clone(),
                        extractions: element.extractions.clone(),
                        landmark_suggestions: element.landmark_suggestions
                            .iter()
                            .map(|suggestion| {
                                if let Some(result) = matching_results.iter().find(|result| result.element.temporary_id == suggestion.temporary_id) {
                                    LandmarkMatching {
