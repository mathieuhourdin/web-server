use crate::entities::{
    resource::{NewResource, resource_type::ResourceType, Resource},
    error::{PpdcError},
...
pub async fn qualify_trace(trace: &str) -> Result<(NewResource, Option<NaiveDate>), PpdcError> {
    ...
    let new_resource = NewResource::new(resource_properties.title, resource_properties.subtitle, trace.to_string(), ResourceType::Trace);
    Ok((new_resource, resource_properties.interaction_date))
}

pub async fn split_trace_into_simple_elements(trace: &str, context: &str, user_biography: &str) -> Result<Vec<SimpleElement>, PpdcError> {
    let system_prompt = format!("System:
    ...
    Tu peux t'appuyer sur un contexte de travail actuel de l'utilisateur pour décomposer le texte.
    ...
    Pour le rephrased_content, propose une reformulation en français, en t'appuyant sur le contexte.
    Si l'element parle d'une resource ou donnée (livre, article, etc.), indique le.
    ...
    let elements: SimpleElements = make_gpt_request(
        system_prompt, 
        user_prompt,
        schema
    ).await.unwrap();
...
pub fn create_resource_from_simple_element(simple_element: &SimpleElement, existing_trace_id: Uuid, user_id: Uuid, analysis_resource_id: Uuid, pool: &DbPool) -> Result<Resource, PpdcError> {
    let content = format!("Extrait de la trace : {}\n\nReformulation : {}", simple_element.content.clone(), simple_element.rephrased_content.clone());
    let new_resource = NewResource::new(simple_element.title.clone(), simple_element.subtitle.clone(), content, ResourceType::Trace);
    ...
    let mut new_resource_relation_to_trace = NewResourceRelation::new(resource.id, existing_trace_id);
    ...
    let mut new_resource_relation_to_analysis = NewResourceRelation::new(resource.id, analysis_resource_id);
...
pub async fn create_element_resources_from_trace(trace: Resource, user_id: Uuid, context: String, analysis_resource_id: Uuid, pool: &DbPool) -> Result<Vec<Resource>, PpdcError> {
    let trace_content = format!("Titre : {}\nSous titre : {}\nContenu : {}", trace.title, trace.subtitle, trace.content);

    let user_biography = get_user_biography(&user_id, pool)?;

    let elements = split_trace_into_simple_elements(trace_content.as_str(), context.as_str(), user_biography.as_str()).await?;
    let resources = elements
    .iter()
    .map(|element| create_resource_from_simple_element(element, trace.id, user_id, analysis_resource_id, pool).unwrap())
    .collect();
    Ok(resources)
}