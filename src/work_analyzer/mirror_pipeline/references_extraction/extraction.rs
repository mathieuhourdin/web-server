use crate::db::DbPool;
use crate::entities::error::PpdcError;
use crate::entities::resource::MaturingState;
use crate::entities_v2::landmark::{Landmark, LandmarkType as EntityLandmarkType, NewLandmark};
use crate::entities_v2::reference::model::Reference;
use crate::entities_v2::{
    reference::model::{NewReference, ReferenceType},
    trace_mirror::TraceMirror,
};
use crate::openai_handler::GptRequestConfig;
use serde::{Deserialize, Serialize};
use uuid::Uuid;

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct ReferencesExtractionResult {
    pub references: Vec<ReferencesExtractionResultItem>,
    pub tagged_text: String,
}

// this is the expected output of the LLM call for reference extraction.
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct ReferencesExtractionResultItem {
    pub tag_id: i32,
    pub mention: String,
    pub landmark_id: Option<i32>,
    pub identification_status: IdentificationStatus,
    pub description: String,
    pub title_suggestion: String,
    pub landmark_type: LandmarkType,
    pub reference_type: ReferenceType,
    pub reference_variants: Vec<String>,
    pub context_tags: Vec<String>,
    pub confidence: f32,
}

#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum IdentificationStatus {
    Matched,
    New,
}

#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum LandmarkType {
    Person,
    Resource,
    Project,
    Organization,
    Text,
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct LandmarkReferenceContextItem {
    #[serde(skip_serializing)]
    pub uuid: Uuid,
    pub landmark_id: i32,
    pub title: String,
    pub subtitle: String,
    pub content: String,
    pub existing_references: Vec<ReferenceContextItem>,
}

#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct ReferenceContextItem {
    pub local_id: i32,
    pub mentions: Vec<String>,
    pub context_tags: Vec<String>,
}

fn to_entity_landmark_type(landmark_type: LandmarkType) -> EntityLandmarkType {
    match landmark_type {
        LandmarkType::Person => EntityLandmarkType::Author,
        LandmarkType::Resource
        | LandmarkType::Project
        | LandmarkType::Organization
        | LandmarkType::Text => EntityLandmarkType::Resource,
    }
}

pub async fn run(
    analysis_id: Uuid,
    trace_mirror_id: Uuid,
    landmarks: &Vec<Landmark>,
    pool: &DbPool,
) -> Result<TraceMirror, PpdcError> {
    let mut trace_mirror = TraceMirror::find_full_trace_mirror(trace_mirror_id, pool)?;
    let context = build_context(landmarks, pool)?;
    let extraction_result = get_references_drafts(&trace_mirror.content, &context, pool).await?;
    for reference in extraction_result.references {
        match reference.identification_status {
            IdentificationStatus::Matched => {
                let landmark_id = reference.landmark_id.unwrap();
                let landmark_uuid = context
                    .iter()
                    .find(|l| l.landmark_id == landmark_id)
                    .unwrap()
                    .uuid;
                let new_reference = NewReference::new(
                    reference.tag_id,
                    trace_mirror_id,
                    Some(landmark_uuid),
                    analysis_id,
                    trace_mirror.user_id,
                    reference.mention,
                    reference.reference_type,
                    reference.context_tags,
                    reference.reference_variants,
                    None,
                    false,
                );
                let _new_reference = new_reference.create(pool)?;
            }
            IdentificationStatus::New => {
                // TODO : a request could be made to GPT to generate the title and subtitle of the new landmark
                let new_landmark = NewLandmark::new(
                    reference.title_suggestion,
                    "".to_string(),
                    reference.description,
                    to_entity_landmark_type(reference.landmark_type),
                    MaturingState::Draft,
                    analysis_id,
                    trace_mirror.user_id,
                    None,
                );
                let new_landmark = new_landmark.create(pool)?;

                let new_reference = NewReference::new(
                    reference.tag_id,
                    trace_mirror_id,
                    Some(new_landmark.id),
                    analysis_id,
                    trace_mirror.user_id,
                    reference.mention,
                    reference.reference_type,
                    reference.context_tags,
                    reference.reference_variants,
                    None,
                    false,
                );
                let _new_reference = new_reference.create(pool)?;
            }
        }
    }
    trace_mirror.content = extraction_result.tagged_text;
    let trace_mirror = trace_mirror.update(pool)?;
    Ok(trace_mirror)
}

pub fn persist_entities(
    trace_mirror_id: Uuid,
    extraction_result: ReferencesExtractionResult,
    pool: &DbPool,
) -> Result<(), PpdcError> {
    let mut trace_mirror = TraceMirror::find_full_trace_mirror(trace_mirror_id, pool)?;
    for reference in extraction_result.references {}
    unimplemented!()
}

pub async fn get_references_drafts(
    text: &String,
    context: &Vec<LandmarkReferenceContextItem>,
    pool: &DbPool,
) -> Result<ReferencesExtractionResult, PpdcError> {
    let system_prompt = include_str!("../prompts/references_extraction/system.md").to_string();
    let user_prompt = format!(
        "trace_text : {}\n\n
        landmarks_to_match : {}",
        text,
        serde_json::to_string(&context)?
    );
    let schema = include_str!("../prompts/references_extraction/schema.json").to_string();
    let gpt_request_config = GptRequestConfig::new(
        "gpt-4.1-mini".to_string(),
        system_prompt,
        user_prompt,
        Some(serde_json::from_str(&schema)?),
        None,
    );
    let result: ReferencesExtractionResult = gpt_request_config.execute().await?;
    Ok(result)
}

pub fn build_context(
    landmarks: &Vec<Landmark>,
    pool: &DbPool,
) -> Result<Vec<LandmarkReferenceContextItem>, PpdcError> {
    let mut landmark_index = 0;
    let mut reference_index = 0;
    let mut context = Vec::new();
    for landmark in landmarks {
        let mut landmark_context = LandmarkReferenceContextItem {
            uuid: landmark.id,
            landmark_id: landmark_index,
            title: landmark.title.clone(),
            subtitle: landmark.subtitle.clone(),
            content: landmark.content.clone(),
            existing_references: Vec::new(),
        };
        let existing_references = Reference::find_for_landmark(landmark.id, &pool)?;
        for reference in existing_references {
            let reference_context = ReferenceContextItem {
                local_id: reference_index,
                mentions: vec![reference.mention.clone()],
                context_tags: reference.context_tags.clone(),
            };
            landmark_context.existing_references.push(reference_context);
            reference_index += 1;
        }
        context.push(landmark_context);
        landmark_index += 1;
    }
    Ok(context)
}
