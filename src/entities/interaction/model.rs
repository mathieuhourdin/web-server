...
impl Interaction {
    pub fn load_paginated(
        ...
        let thought_outputs = query
            ...
            .into_iter()
            .map(|(interaction, resource)| InteractionWithResource {
                interaction,
                resource,
            })
            .collect();
    ...
    pub fn find_all_draft_traces_for_user_before_date(
        user_id: Uuid,
        date: NaiveDate,
        pool: &DbPool,
    ) -> Result<Vec<InteractionWithResource>, PpdcError> {
    ...
    pub fn find_all_traces_for_user_after_date(
        user_id: Uuid,
        date: NaiveDateTime,
        pool: &DbPool,
    ) -> Result<Vec<InteractionWithResource>, PpdcError> {
    ...
    pub fn find_first_trace_for_user(
        user_id: Uuid,
        pool: &DbPool,
...
mod tests {
    ...
    fn test_find_paginated_outputs_problems() {
        ...
        let interactions =
            Interaction::find_paginated_outputs_problems(0, 10, uuid, &pool).unwrap();
