...
impl HttpResponse {
    ...
    pub fn unauthorized() -> HttpResponse {
        HttpResponse::new(
            ...
            "User should be authenticated".to_string(),
        )
    }
    ...
    pub fn from_file(status_code: StatusCode, file_path: &str) -> Result<HttpResponse, PpdcError> {
        ...
        let html_body = html_body.replace(
            "process.env.API_URL",
            format!("'{}'", get_api_url()).as_str(),
        );
    ...
    pub fn set_header(&mut self, header_title: &str, header_content: String) {
        self.headers
            .insert(String::from(header_title), header_content);
    }
